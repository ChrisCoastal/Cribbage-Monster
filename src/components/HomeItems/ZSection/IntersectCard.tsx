...
const IntersectCard: FC<IntersectCardProps> = ({ image, title, text, layout = 'lr', children }) => {
  ...
  const isIntersect = useIntersectionObserver(cardRef, { threshold: 0.5 });
  const animate = isIntersect ? 'opacity-1 translate-y-0' : 'opacity-0 translate-y-12';
  const direction = layout === 'lr' ? 'flex-row' : 'flex-row-reverse';

  return (
    <>
      <li
        ref={cardRef}
        className={`${animate} flex flex-col sm:${direction} gap-6 transition-all duration-1000 sm:gap-10`}>
